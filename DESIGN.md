...
- Findings: 기본 정렬은 `CRITICAL`, `HIGH`, `MEDIUM`, `LOW`, `UNKNOWN`, 상대 파일 경로, 시작 줄, Finding ID 순이다. 목록 필터는 심각도, KISA 매핑 상태, 언어만 제공하며, 메시지·탐지 근거·코드 조각·조치 권고는 상세 패널에서 표시한다. 결과 상세는 왼쪽 목록과 오른쪽 조회 패널을 함께 보이고 두 영역은 독립적으로 스크롤한다. 패널을 닫아도 목록의 필터·페이지·스크롤 위치를 유지한다. 상세 패널은 심각도와 진단 항목명을 먼저, 파일 경로와 줄 번호를 다음으로 표시한다. KISA 매핑 여부와 무관하게 ADMIN은 원본 분석 결과를 확인할 수 있고, USER에게는 원본 분석 결과를 렌더링하지 않는다. 분석 실행 로그는 Finding 패널이 아니라 FAILED 분석 상태 화면에서 ADMIN에게만 표시한다.
- Catalog management: 공식 KISA 49개 항목은 초기 시드로 제공한다. 목록은 KISA 코드·명칭·분류 검색과 `지원`·`부분 지원`·`미지원` 필터를 제공한다. ADMIN은 목록에서 항목을 선택해 오른쪽 상세 패널에서 설명, 참조 정보, 활성 여부, 기본 심각도, 구현 상태와 조치 권고를 수정하고, 신규 진단 기준을 등록할 수 있다. 같은 `kisa_code`의 중복 등록은 허용하지 않는다. USER는 같은 목록과 상세를 읽기 전용으로 조회한다.
- Action drawers: 소스 등록과 분석 실행, 진단 기준 등록, 프로젝트 생성, 사용자 접근권한 관리는 오른쪽 액션 패널로 연다. 진단 기준 등록은 KISA 코드·명칭·분류·기본 심각도를 필수로 받고, 조치 권고는 기존 항목 수정에서만 받는다. 프로젝트 생성은 이름을 필수, 설명을 선택으로 받으며 언어 선택을 두지 않는다. 결과와 카탈로그의 조회 상세 패널에는 이 규칙을 강제하지 않는다.
...
## Interaction states
- Loading: 업로드·분석 실행·폴링 중 진행 상태를 표시한다. 분석 상세는 `PENDING`, `RUNNING` 동안 3초 간격으로 상태를 갱신하고, 완료·실패·화면 이탈 때 폴링을 중단한다.
- Empty: 프로젝트 없음, 분석 이력 없음, 분석 결과 없음, 필터 결과 없음 안내를 구분한다. 필터 결과 없음에는 필터 초기화 행동을 제공한다.
- Error: 일반 사용자는 일반적인 실패 안내를 보고, 관리자는 허용된 상세 오류 정보를 확인하며 재시도 방법을 제공한다. 인증 실패 또는 만료(401)는 `로그인 정보가 만료되었거나 유효하지 않습니다. 다시 로그인해 주세요.` 안내 뒤 로그인 화면으로 이동한다. 관리자 전용 기능 요청 거부(403)는 `이 기능은 관리자만 사용할 수 있습니다.`를, 프로젝트 자원 미발견 또는 비인가(404)는 `요청한 정보를 찾을 수 없습니다.`를 표시한다. 소스 업로드 오류는 코드별로 `100MB 이하 ZIP만 업로드할 수 있습니다.`, `ZIP 압축 해제 제한을 초과했습니다.`, `안전하지 않은 ZIP입니다.`, `지원하는 소스 파일이 없습니다.`, `분석이 끝난 뒤 업로드할 수 있습니다.`, `다른 업로드가 진행 중입니다.`, `업로드에 실패했습니다. 다시 시도해 주세요.` 중 하나만 짧게 표시하며 내부 경로와 서버 예외는 표시하지 않는다. 분석 실행의 `SOURCE_UPLOAD_IN_PROGRESS`는 `소스 업로드가 진행 중입니다. 완료 후 다시 시도해 주세요.`를 표시하고, 분석 상태 조회의 네트워크 또는 5xx 오류는 분석 실패로 바꾸지 않고 수동 새로고침을 제공한다.
...
- Test/screenshot expectations: 주요 사용자 흐름과 상태별 렌더링을 검증한다. E0 Pen 초안은 다크 톤, 목록·상세 패널 구조, 독립 스크롤, 400px ActionDrawer와 정보 순서의 수동 대조에 사용하며 픽셀 단위 일치는 요구하지 않는다. Pen 초안과 API 계약이 충돌하면 API 계약과 ADR을 우선한다.
