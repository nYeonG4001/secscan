...
E5는 E4가 실행한 Semgrep JSON 결과를 엔진 독립적인 진단 결과로 정규화하고, 가능한 결과를 KISA 카탈로그와 연결해 Finding으로 저장한다. 공통 구조와 보존 규칙은 먼저 확정했고, E4의 고정 Semgrep OSS 1.95.0 실행 결과를 확인한 뒤 초기 매핑·필드·fixture 계약을 구체화한다.
...
- 완료 조건:
  ...
  - 초기 매핑 시드는 `semgrep/secscan.java.runtime-exec → KISA-005`, `semgrep/secscan.javascript.eval → KISA-002`, `semgrep/secscan.python.pickle-loads → KISA-043`이다.
