...
이 문서는 구현 계획이다. 여기에 적은 예정 규칙, KISA 연결, fixture 기대값은 실제 Semgrep 실행과 CI 증거가 생기기 전까지 카탈로그 상태나 요구사항 충족을 뜻하지 않는다. 구현을 시작하기 전 `docs/원본_요구사항목록.docx`를 확인해 여섯 기존 KISA 코드와의 연결을 다시 대조한다. 해당 원본 문서는 현재 작업 환경에 없으므로, 없음을 추정으로 대체하지 않는다.
...
| 규칙 식별자 | source-to-sink 계약별 독립 `engine_rule_id` | 새 API를 기존 rule의 `pattern-either`에 합치지 않는다. |

## 구현 전 spike와 중단 조건

`innerHTML +=`는 일반 구현 순서에 넣지 않는다. 먼저 production 규칙·시드·카탈로그 상태를 바꾸지 않는 spike로 고정 Semgrep OSS의 복합 대입 sink 매칭을 확인한다.

| spike 판정 | 다음 조치 |
|---|---|
| 취약 fixture에서 예정 `check_id`가 정확히 1건, 고정값 정상 fixture에서 0건이며 기존 `innerHTML =` 규칙과 중복하지 않음 | ADR-041과 이 계획을 갱신한 뒤, 아래 일반 전환 게이트를 적용해 별도 독립 규칙 후보로 검토한다. |
| 복합 대입을 sink로 재현성 있게 매치하지 못함, 중복 매치함, 또는 정상 fixture가 불명확함 | 이 확장 라운드에서 제외한다. 대체 문법으로 범위를 넓히지 않는다. |
...
원본 요구사항 문서의 연결, Semgrep 표현력, 기대 fixture 중 하나라도 이 계획의 source-to-sink 계약을 지지하지 않으면 해당 후보를 제외하고 사용자에게 범위 재결정을 요청한다. 기대값을 낮추거나 `부분 지원` 상태를 먼저 바꾸지 않는다.
...
|---|---|---|---|---|
| 1 | `secscan.python.pickle-load` | KISA-043 | `pickle.load($DATA)` | `pickle.loads($DATA)`와 입력 형태와 sink가 다르다. |
| 2 | `secscan.python.path-open` | KISA-003 | `Path($PATH).open(...)` | bare `open($PATH, ...)`와 호출자·sink가 다르다. |
| 3 | `secscan.javascript.function-constructor` | KISA-002 | 한 인자 `Function($CODE)` | 기존 JavaScript `eval($CODE)`과 실행 API가 다르다. |
| 4 | `secscan.javascript.dom-insert-adjacent-html` | KISA-004 | `$ELEMENT.insertAdjacentHTML($POSITION, $DATA)`의 HTML 인자 | 직접 `innerHTML = $DATA` 대입과 sink 문법이 다르다. |
| 5 | `secscan.java.process-builder` | KISA-005 | `new ProcessBuilder($COMMAND).start()` | `Runtime.getRuntime().exec($COMMAND)`와 프로세스 생성 API가 다르다. |
| 6 | `secscan.python.subprocess-run-shell` | KISA-005 | `subprocess.run($COMMAND, shell=True)` | `os.system($COMMAND)`과 API와 `shell=True` 제약이 다르다. |

각 규칙마다 다음 순서를 반복한다.

1. 원본 요구사항 문서와 ADR-041을 대조해 KISA 연결과 제외 범위를 확인한다.
2. 독립 `engine_rule_id`와 고정 source-to-sink 문법을 선언한다. 기존 규칙 ID에 sink를 병합하지 않는다.
3. 취약 fixture와 같은 sink의 고정값 정상 fixture를 작성한다. 정상 fixture는 sanitizer·타입 안전성·모든 API 변형의 안전을 주장하지 않는다.
4. 고정 YAML과 `--no-rewrite-rule-ids`로 Semgrep OSS 1.95.0을 직접 실행해 취약 fixture의 접두어 없는 `check_id` 1건과 정상 fixture의 대상 규칙 0건을 확인한다.
...
2. 취약 fixture는 실제 Semgrep 실행에서 대상 접두어 없는 `check_id`를 정확히 1건 낸다.
