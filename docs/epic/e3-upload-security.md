...
- 테스트: `backend/tests/test_upload_security.py`
- 증거: 각 제한 초과 입력의 거부 응답과 처리 중단 뒤 작업영역 부재 확인
...
- 테스트: `backend/tests/test_source_upload_api.py`, `backend/tests/test_upload_security.py`
- 증거: ADMIN과 USER 요청, CSRF 누락, 분석 상태별 409 또는 성공 응답, 업로드 뒤 분석 미생성 확인
...
- 테스트: `frontend/src/pages/SourceUploadDrawer.test.tsx`, `backend/tests/test_source_upload_api.py`, `backend/tests/test_upload_security.py`
- 증거: 오류 범주별 화면, 취소 뒤 상태 조회, USER 비노출, 실패 뒤 서버 임시 경로 부재 확인
...
| 정상 업로드 | Java, JavaScript, Python 단일 언어와 복수 언어 ZIP 등록, 감지 언어 기록, 분석 미자동 실행 | 없음 | `backend/tests/test_source_upload_api.py`, `backend/tests/test_source_language_detection.py` |
...
- 테스트: `backend/tests/test_upload_security.py`
- 증거: ZIP Slip, 절대 경로, Windows 경로, NUL 문자, 문자 단위 중복 경로, 대소문자만 다른 경로, 심볼릭 링크와 하드 링크 입력의 전체 거부와 작업영역 정리 확인
...
- 테스트: `backend/tests/test_source_language_detection.py`, `backend/tests/test_upload_security.py`
- 증거: 단일 언어와 복수 언어 ZIP의 `target_languages`, 지원 소스 부재 ZIP의 거부 응답, 분석 입력 파일 목록 확인
