...
- 결정과 근거: 원본 ZIP 크기만 제한하면 압축 해제 뒤의 자원 고갈을 막을 수 없다. ZIP 원본 100 MiB, 실제 압축 해제 누적 300 MiB, 최대 10,000개 파일, 개별 파일 25 MiB를 제한한다(2026-09-04 교육용 테스트 소스 수용을 위해 네 한도를 함께 상향; ADR-015 참고). ZIP 항목별 압축 비율은 스트리밍 실측값으로 계산하고 실제 출력이 1 MiB 이상일 때만 20:1 제한을 적용한다. 실제 압축 해제 크기 제한은 비율과 별도로 적용한다. 자세한 근거는 ADR-015를 따른다.
- 완료 조건:
...
- 자원 제한 테스트는 100 MiB, 300 MiB 같은 운영 기본값을 실제로 채우지 않는다. 테스트 전용 설정에서 더 작은 제한값을 주입해 같은 경계 동작을 빠르게 검증한다.
