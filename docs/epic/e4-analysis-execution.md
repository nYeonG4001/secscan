...
E4 실행 범위는 구현·검증을 마쳤다. 전용 PostgreSQL에서 Alembic `0007`까지 upgrade, downgrade base, 재-upgrade를 확인했고, backend 전체 pytest 195개와 `ruff check app tests`, frontend lint·30개 Vitest·typecheck·production build, non-root·named volume·1.5 GiB Compose 제한·Semgrep OSS 1.95.0 Docker 검증을 통과했다.

이 에픽은 분석 실행·상태·안전한 실행 메타데이터까지만 책임진다. 개별 Semgrep 결과의 정규화·KISA 매핑·Finding 저장은 E5, 결과 목록과 상세 UI의 완성은 E6 범위다. E5는 고정 YAML 직접 지정과 taint 규칙 보완을 별도 변경으로 수행한다.
...
- 상태: 완료, E5 규칙 품질 보완 이관 (2026-08-28)
...
- 완료 조건:
  ...
  - Semgrep JSON 출력의 형식과 결과 배열을 검증하고, 안전한 실행 메타데이터만 Analysis에 저장한다. 개별 결과 정규화·저장은 E5가 담당한다.
  - 자체 규칙 변경은 별도 PR, KISA/CWE/OWASP 근거 검토, 영향받는 기대 결과 테스트 갱신을 거친다.
  - Semgrep 공식 또는 다른 제3자 규칙을 복사·변형하지 않는다.
- 후속 이관: 고정 Semgrep OSS CLI는 `1.95.0`으로 기록했다. E5는 규칙 YAML 직접 지정, source-to-sink taint 규칙, 실제 KISA 매핑 fixture를 구현한다.
...
- 상태: 완료, 결과 목록은 E5/E6 이관 (2026-08-28)
...
- 완료 조건:
  ...
  - `COMPLETED`는 결과 정규화와 결과 UI가 E5/E6에서 이어진다는 완료 안내를, `FAILED`는 역할별 실패 안내를 같은 분석 상세 경로에서 표시한다.
...
- 완료 조건:
  ...
  - 프론트에서 분석 생성 뒤 상태 화면 이동, `PENDING`·`RUNNING` 3초 폴링, 완료 안내, 실패 역할 분기, 401·5xx 처리, 두 409 코드별 행동을 검증한다.
- 테스트 파일:
  ...
  - `frontend/src/App.test.tsx`: 상태 폴링, 역할별 실패 화면, 분석 시작, 버튼 상태, 두 409 코드 처리
  - Docker 검증: 고정 Semgrep CLI 버전, non-root 사용자, named volume 저장소 쓰기 권한
- 증거: 2026-08-28 backend 전체 pytest 195개, `ruff check app tests`, frontend 30개 Vitest·lint·typecheck·build, Alembic 0007 upgrade/downgrade, Docker non-root·named volume·Semgrep 1.95.0·Compose 메모리 제한 검증

## 이후 작업

- E5: 개별 Semgrep JSON 결과 정규화, YAML 직접 지정, taint 규칙 보완, KISA 매핑과 Finding 저장
- E6: 실제 Finding 결과 목록·필터·상세 UI
