...
E3부터는 하나의 작업 폴더를 여러 에이전트가 동시에 수정하지 않는다. 에픽 구현 시작 전에 다음 환경을 만든다.

1. E2까지 확정한 공통 설계 문서를 에픽 구현 브랜치에 반영한다.
2. 에픽 통합 브랜치를 기준으로 작업 묶음별 Git worktree와 작업 브랜치를 만든다.
3. 에이전트마다 담당 작업 번호와 수정 가능 파일 범위를 고정한다. 같은 파일의 동시 수정은 배정하지 않는다.
4. 각 작업 브랜치는 담당 테스트와 최소 검증을 통과한 뒤 통합 브랜치로 반영한다.
5. 통합 브랜치에서 전체 테스트와 CI를 다시 확인한 뒤 에픽 구현 PR 하나를 만든다.

예를 들어 E3에서는 업로드 API, ZIP 검증, 프론트 업로드 상태처럼 파일 경계가 다른 작업을 병렬 배정할 수 있다. 인증 공통 의존성처럼 여러 작업이 동시에 수정하는 기반 파일은 먼저 단일 작업으로 확정한 뒤 후속 작업을 시작한다.
...
3. 요구사항을 해석하거나 기술 선택을 하는 중요한 결정은 아래 내용을 남긴다.
   - 요구사항을 어떻게 이해했는지
   - 실제로 고려한 대안
   - 선택한 방향과 안전성, 사용자 경험, 일정, 확장성 측면의 이유
   - 이번 에픽에서 구현하는 범위와 다음 에픽으로 미루는 범위
   - ADR 또는 `requirements-interpretation.md`의 기록 위치
4. 멘토 또는 고객에게 질문할 때는 특정 기술의 채택 여부보다 해결하려는 문제, 기대 효과, 선택지의 영향을 먼저 평이한 언어로 설명한다.
5. 작업별 완료 조건을 검증 가능한 문장으로 작성한다.
6. 허용 시나리오와 거부 시나리오를 포함한 테스트 종류와 경로를 정한다.
7. API 응답, 테스트 로그, 화면 캡처 등 완료 증거를 정한다.
8. 상세화한 작업을 구현하고 완료한 뒤 검증표의 상태와 증거를 갱신한다.
...
- 사용자에게 보여 주는 설명과 저장소 문서는 한국어로 작성한다.
- Codex와 Claude, Codex와 Codex 사이의 인계 프롬프트와 작업 결과 보고는 별도 변경 전까지 간결한 영어로 작성한다. 관리 담당 에이전트가 사용자에게 한국어로 결과를 정리한다.
- 인계문에는 요구사항 원문이나 긴 로그를 복사하지 않는다. 작업 목표, 금지 범위, 읽을 문서 경로, 완료 조건, 검증 명령, 현재 브랜치와 미커밋 변경 여부만 적는다.
- 새 에이전트에게 전체 저장소를 읽히지 않는다. `AGENTS.md`, 해당 에픽 문서, 담당 파일과 의존하는 ADR만 읽힌다. 요구사항 해석이나 교차 영역 판단이 필요할 때만 읽기 범위를 넓힌다.
- 작업 중 진행 보고는 장황한 서술 대신 `changed`, `validated`, `blocked` 세 항목으로 끝낸다. 다음 행동에 필요한 오류와 근거만 남긴다.
- 요구사항 해석, 보안 정책, 코드 리뷰에는 고성능 모델을 사용한다. 범위가 확정된 구현, 문서 형식 정리, 단일 검증에는 비용이 낮은 모델을 우선 사용한다.
- 재사용 가능한 작업 지시는 `docs/templates/agent-handoff.md`를 기준으로 작성한다.
