...
erDiagram
    ...
    ANALYSIS {
        ...
        string source_location "분석 생성 시점의 내부 원본 위치"
...
`ANALYSIS.source_location`은 분석 생성 커밋 시점의 `PROJECT.source_location`을 보존하는 내부 실행 입력값이다. API 응답과 `raw_result`에는 포함하지 않는다.
